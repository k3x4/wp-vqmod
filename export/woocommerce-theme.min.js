...
if(typeof variation.image_src=='undefined'){return}
var $product=$(this).closest('.product, .product-col'),$image_slider=$product.find('.product-image-slider'),$thumbs_slider=$product.find('.product-thumbs-slider'),links;var $imageItems=$image_slider.find('.owl-item');var cloned=$image_slider.find('.owl-item.cloned').length/2;var variationImgIndex=0;$image_slider.find('.owl-item:not(.cloned) img').each(function(){if($(this).attr('src')===variation.image_src){variationImgIndex=$imageItems.index($(this).closest('.owl-item'))-cloned;return}});if($image_slider.length){$image_slider.trigger('to.owl.carousel',[variationImgIndex,1,!0])}
if($thumbs_slider.length){$thumbs_slider.trigger('to.owl.carousel',[variationImgIndex,1,!0]);$thumbs_slider.find('.owl-item:eq('+variationImgIndex+')').click()}});var porto_fb_update_trigger=null;$(document).on('found_variation reset_image','.variations_form',function(event,variation){if($(this).closest('.fancybox-inner').length&&$.fancybox){$(window).off('resize.fb',$.fancybox.update);if(porto_fb_update_trigger){theme.deleteTimeout(porto_fb_update_trigger)}
...
$(document).on('tabactivate','.woocommerce-tabs',function(e,ui){var label=$(ui).attr('aria-controls');var panel=$('[aria-labelledby="'+label+'"');theme.refreshVCContent(panel)})})}).apply(this,[window.theme,jQuery]);(function(theme,$,undefined){$(document).ready(function(){theme.WooVariationForm.initialize();theme.WooProductImageSlider.initialize();porto_woocommerce_init();$(window).on('vc_reload',function(){porto_woocommerce_init();$('.type-product').addClass('product')});if(!$('#yith-wcwl-popup-message').length){$('body').prepend($('<div>').attr('id','yith-wcwl-popup-message').html('<div id="yith-wcwl-message"></div>').hide())}
...
if($form.find('input[name="category"]').val()&&typeof $form.data('original_cat_id')=='undefined'){$form.data('original_cat_id',$form.find('input[name="category"]').val())}
...
if($.isFunction($.fn.themeWooWidgetAccordion)){$(function(){$wrap.find('.widget_product_categories, .widget_price_filter, .widget_layered_nav, .widget_layered_nav_filters, .widget_rating_filter').each(function(){var $this=$(this),opts;var pluginOptions=$this.data('plugin-options');if(pluginOptions)
